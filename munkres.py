...
#    pass
DISALLOWED = -99999#float('Inf')#DISALLOWED_OBJ()
...
class Munkres:
    ...
    def __step1(self) -> int:
        ...
        n = self.n
        for i in range(n):
            vals = [x for x in self.C[i] if x != DISALLOWED]
            if len(vals) == 0:
        ...
        for i in range(self.n):
            for j in range(self.n):
                if (not self.row_covered[i]) and (not self.col_covered[j]):
                    if self.C[i][j] != DISALLOWED and minval > self.C[i][j]:
                        minval = self.C[i][j]
        return minval
...
if __name__ == '__main__':
    ...
    print_matrix(df, msg='cost matrix')
